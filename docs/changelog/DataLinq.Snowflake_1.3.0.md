...
When running categorical flows (like `Cases().SelectCase()`), DataLinq automatically caches the evaluated base query into session-scoped `TEMPORARY TABLE` structures. This completely eliminates N+1 re-evaluation penalties without producing deferred pipeline memory leaks.
